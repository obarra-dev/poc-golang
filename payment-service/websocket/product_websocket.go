package websockets

import (
	"fmt"
	"log"
	"net/http"
	"time"

	"golang.org/x/net/websocket"

	"poc-golang/payment-service/webservices/product"
)

type message struct {
	Data string `json:"data"`
	Type string `json:"type"`
}

func SetupRoutes(apiBasePath string) {
	http.Handle(fmt.Sprintf("%s/%s", apiBasePath, "products"), websocket.Handler(productSocket))
}

func productSocket(ws *websocket.Conn) {
	// we can verify that the origin is an allowed origin
	fmt.Printf("origin: %s\n", ws.Config().Origin)

	done := make(chan struct{})
	go func(c *websocket.Conn) {
		for {
			//var msg message
			msg := message{}
			if err := websocket.JSON.Receive(ws, &msg); err != nil {
				log.Println("Error recieving message")
				log.Println(err)
				break
			}
			fmt.Printf("recieved message %s\n", msg.Data)
		}
		close(done)
	}(ws)

loop:
	for {
		select {
		case <-done:
			fmt.Println("connection was closed, lets break out of here")
			break loop
		default:
			fmt.Println("sending product list to the client")

			products, err := product.GetProductList()
			if err != nil {
				log.Fatal(err)
			}
			if err := websocket.JSON.Send(ws, products); err != nil {
				log.Println(err)
				break
			}
			// pause for 10 seconds before sending again
			time.Sleep(10 * time.Second)
		}
	}
	fmt.Println("closing the connection")
	defer ws.Close()
}
